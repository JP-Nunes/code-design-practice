...
class PurchaseTest {
    ...
    inner class TotalWithDiscountPercentage {
        ...
        fun `should apply 10 percent discount`() {
            val purchase = purchase(total = bd("100.00"), percent = bd("10"))

            assertThat(purchase.totalWithDiscount).isEqualByComparingTo(bd("90.00"))
        }

        @Test
        fun `should keep same total when discount is 0 percent`() {
            val purchase = purchase(total = bd("100.00"), percent = bd("0"))

            assertThat(purchase.totalWithDiscount).isEqualByComparingTo(bd("100.00"))
        }

        @Test
        fun `should be zero when discount is 100 percent`() {
            val purchase = purchase(total = bd("100.00"), percent = bd("100"))

            assertThat(purchase.totalWithDiscount).isEqualByComparingTo(bd("0.00"))
        }

        @Test
        fun `should return total when there is no voucher`() {
            val purchase = purchase(total = bd("123.45"), percent = null)

            assertThat(purchase.totalWithDiscount).isEqualByComparingTo(bd("123.45"))
        }
    }
